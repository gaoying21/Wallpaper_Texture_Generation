  ...
  # the entire image.
    ...
    if not thread_id:
      tf.summary.image('final_distorted_image',
                       tf.expand_dims(distorted_image, 3))
...
def parse_example_proto(example_serialized):
  ...
  label = tf.reshape(features['image/class/label'].values,[1,39])
  ...
  with tf.name_scope('batch_processing'):
    ...
    if train:
      examples_queue = tf.RandomShuffleQueue(
          capacity=min_queue_examples + 64 * batch_size,
    ...
    return images, tf.reshape(label_index_batch, [batch_size,39])
