...
def inference(images, num_classes, for_training=False, restore_logits=True,
  ...
  Args:
    ...
    for_training: If set to `True`, build the inference model for training.
      ...
      e.g. dropout, are appropriately fconfigured.
  ...
  with slim.scopes.arg_scope([slim.ops.conv2d, slim.ops.fc], weight_decay=0.00000):
...
def loss(logits, labels, batch_size=None):
  ...
  slim.losses.euclidean_loss(logits,
                                 labels,
                                 weight=1.0)

  # Cross entropy loss for the auxiliary softmax head.
#  slim.losses.euclidean_loss(logits[1],
#                                 labels,
#                                 weight=0.4,
#                                 scope='aux_loss')
