...
class DCGAN(object):
  ...
  def build_model(self,batch_size,images,labels,numclasses):

    self.z = tf.random_uniform([batch_size,self.z_dim], -1, 1, dtype=tf.float32)
    
    tf.summary.histogram('z', self.z)
    
    self.G = self.generator(y=labels, y_dim=numclasses, is_training=True, restore=False, scope='g')
    self.D = self.discriminator(images, reuse=False, is_training=True, restore=False, scope='d')

    self.D_ = self.discriminator(self.G, reuse=True, is_training=True, restore=False, scope='d')
    ...
    self.g_loss_d = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_, labels=tf.ones_like(self.D_)))
    self.g_loss_h = self.perception_loss(self.G, labels, numclasses, False, True, 'h')
    
    self.g_loss = self.g_loss_d + self.g_loss_h
...
#    self.d_total_loss = tf.add_n([self.d_loss] + self.d_regularization_losses, name='d_total_loss')
  ...
  def train(self):
    """Train DCGAN"""
    with tf.Graph().as_default(), tf.device('/cpu:1'):
      ...
      with tf.device('/gpu:1'):
        ...
        self.build_model(FLAGS.batch_size, images, labels, 39)
  ...
  def evaluate(self):
    """Evaluate DCGAN"""
    csvfile=file('22_39.csv','rb')
#   csvfile=file('wallpaperNor.csv','rb')
      ...
      with tf.device('/cpu:0'):
        ...
        with slim.scopes.arg_scope([slim.ops.conv2d, slim.ops.deconv2d, slim.ops.fc], 
          ...
          with slim.scopes.arg_scope([slim.ops.conv2d, slim.ops.deconv2d, slim.ops.fc, slim.ops.batch_norm, slim.ops.dropout],
            ...
            labels = tf.placeholder(tf.float32, (FLAGS.batch_size, 39))
            self.G = self.generator(y=labels, y_dim=39, is_training=False, restore=True, scope='g')
...
#        variables_to_restore = ema.variables_to_restore()
      ...
      for l in unique_labels:
        samples = sess.run(self.G, feed_dict={labels:np.tile(l,(FLAGS.batch_size,1)), self.z:np.random.uniform(-1,1,(FLAGS.batch_size,self.z_dim))})
        save_images(samples, './%s/%d' % (FLAGS.result_dir, k))
        k = k+1
        
  def discriminator(self, image, reuse, is_training, restore, scope):
    batch_norm_params = {'decay': BATCHNORM_MOVING_AVERAGE_DECAY,'epsilon': 0.001}
  ...
  def generator(self, y, y_dim, is_training, restore, scope):
    ...
    #        h11 = slim.ops.conv2d(h10, 32, [1, 1], scope='proj4')
            ...
            h6 = slim.ops.deconv2d(h5, [FLAGS.batch_size, 299, 299, 3], [5, 5], stride=2, stddev=0.2, scope='deconv7', activation=tf.nn.tanh, batch_norm_params=None)
  ...
  def perception_loss(self, images, labels, num_classes, is_training, restore, scope):
    ...
    with tf.name_scope(scope) as scope:
      with tf.device('/gpu:1'): 
         ...
         logits = inception.inference(images, num_classes, for_training=is_training, restore_logits=restore, scope=scope)
