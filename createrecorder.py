...
tf.app.flags.DEFINE_string('data_directory', 'texturecrop_wallpaper',
                           'Training data directory')
tf.app.flags.DEFINE_string('output_directory', 'recorderdata/',
                           'Output data directory')

tf.app.flags.DEFINE_integer('train_shards', 480,
                            'Number of shards in training TFRecord files.')
tf.app.flags.DEFINE_integer('validation_shards', 20,
...
tf.app.flags.DEFINE_string('labels_file', 'adjprerelu7Nor.csv', 'Labels file')
...
def _convert_to_example(filename, image_buffer, label, height, width):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/filename': _bytes_feature(b'os.path.basename(filename)'),
...
class ImageCoder(object):
  ...
  def __init__(self):
    ...
    self._png_to_jpeg = tf.image.encode_jpeg(image, format='', quality=100)
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames, labels, num_shards):
  ...
  for s in range(num_shards_per_batch):
    ...
    for i in files_in_shard:
      ...
      temp = re.split('_',os.path.basename(filename))
      label = labels[int(temp[0])-1]
...
def _find_image_files(data_dir, labels_file):
  ...
  return filenames, unique_labels
...
def _process_dataset(directory, labels_file):
  ...
  trainfilenames=filenames[0:43200]
  valfilenames=filenames[43200:]
  _process_image_files('train', trainfilenames, labels, FLAGS.train_shards)
  _process_image_files('validation', valfilenames, labels, FLAGS.validation_shards)
